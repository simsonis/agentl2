...
CREATE TABLE form_templates (
    ...
    embedding_vector VECTOR(1536)  -- OpenAI text-embedding-ada-002 차원
);
...
COMMENT ON COLUMN form_templates.template_type IS '서식 유형 (1001=비밀유지계약서)';
COMMENT ON COLUMN form_templates.embedding_vector IS 'LLM 의미 검색용 임베딩 벡터';
...
CREATE TABLE form_paragraphs (
    ...
    tags TEXT[],
    embedding_vector VECTOR(768),  -- 문단용은 작은 차원 사용
...
DO $$
BEGIN
    RAISE NOTICE '✅ 서식 관리 DB 스키마 생성 완료!';
